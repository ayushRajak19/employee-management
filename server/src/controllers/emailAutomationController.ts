...
export const getBroadcastAudience = async (request: Request, response: Response): Promise<void> => { requireSuperAdmin(request); response.json({ success: true, message: "Broadcast audience preview retrieved", data: await service.previewBroadcastAudience(typeof request.query.source === "string" ? request.query.source : undefined) }); };
export const getBroadcasts = async (request: Request, response: Response): Promise<void> => { requireSuperAdmin(request); response.json({ success: true, message: "Broadcasts retrieved", data: { items: await service.listBroadcasts() } }); };
export const createBroadcast = async (request: Request, response: Response): Promise<void> => { requireSuperAdmin(request); response.status(201).json({ success: true, message: "Broadcast queued", data: await service.createBroadcast(request.body, request.user!.id) }); };
export const cancelBroadcast = async (request: Request, response: Response): Promise<void> => { requireSuperAdmin(request); response.json({ success: true, message: "Broadcast cancelled", data: await service.cancelBroadcast(String(request.params.id), request.user!.id) }); };
