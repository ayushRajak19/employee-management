...
export const geographyHeatmapPoints = async (request: Request, response: Response) => send(response, "Geographic heatmap points retrieved", await geographicSales.getGeographicHeatmapPoints(request.user!, (request.query.type as "leads" | "customers" | "revenue" | "quantity") || "leads", request.query.geoId ? String(request.query.geoId) : undefined));
