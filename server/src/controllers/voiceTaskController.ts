...
export const previewAudio = async (request: Request, response: Response): Promise<void> => {
  ...
  const transcription = await service.transcribeAudio(request.file);
...
export const previewText = async (request: Request, response: Response): Promise<void> => { response.status(201).json({ success: true, message: "Command interpreted", data: await service.createPreview(request.body.transcript, actor(request)) }); };
