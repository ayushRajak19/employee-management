...
export const previewAudio = async (request: Request, response: Response): Promise<void> => {
  ...
  const timezoneOffsetMinutes = Number(request.body.timezoneOffsetMinutes);
  response.status(201).json({ success: true, message: "Voice transcribed", data: await service.createPreview(transcription.text, actor(request), { ...transcription, timezoneOffsetMinutes: Number.isFinite(timezoneOffsetMinutes) ? timezoneOffsetMinutes : 0 }) });
};
export const previewText = async (request: Request, response: Response): Promise<void> => { response.status(201).json({ success: true, message: "Command interpreted", data: await service.createPreview(request.body.transcript, actor(request), { language: request.body.language === "auto" ? undefined : request.body.language, timezoneOffsetMinutes: request.body.timezoneOffsetMinutes }) }); };
