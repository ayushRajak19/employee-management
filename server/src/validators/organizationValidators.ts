...
export const departmentSchema = z.object({ body: z.object({ name: z.string().trim().min(2).max(120), code: code.optional(), description: z.string().trim().max(500).optional(), capabilities }) });
export const teamSchema = z.object({ body: z.object({ name: z.string().trim().min(2).max(120), code: code.optional(), department: objectId, description: z.string().trim().max(500).optional() }) });
export const designationSchema = z.object({ body: z.object({ name: z.string().trim().min(2).max(120), code: code.optional(), department: objectId.optional(), level: z.string().trim().max(50).optional(), description: z.string().trim().max(500).optional(), catalogRole: z.string().trim().max(120).optional() }) });
