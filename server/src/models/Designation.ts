...
export interface DesignationDocument { name: string; code: string; department?: Types.ObjectId; level?: string; description?: string; requiredSkills: RequiredSkill[]; isActive: boolean; archivedAt?: Date }
...
const schema = new Schema<DesignationDocument>({
  ...
  department: { type: Schema.Types.ObjectId, ref: "Department", index: true }, level: { type: String, trim: true, maxlength: 50 }, description: { type: String, trim: true, maxlength: 500 },
