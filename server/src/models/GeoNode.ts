...
export interface GeoNodeDocument { name: string; code: string; type: GeoNodeType; parent?: Types.ObjectId; ancestors: Types.ObjectId[]; depth: number; location?: { type: "Point"; coordinates: [number, number] }; boundary?: { type: "Polygon" | "MultiPolygon"; coordinates: unknown[] }; isActive: boolean }
const schema = new Schema<GeoNodeDocument>({
  ...
  parent: { type: Schema.Types.ObjectId, ref: "GeoNode", index: true }, ancestors: [{ type: Schema.Types.ObjectId, ref: "GeoNode" }], depth: { type: Number, required: true, min: 0, max: 6 },
