...
export const updateMyProfile = async (userId: string, input: { firstName?: string; lastName?: string; phone?: string; professionalSummary?: string; personal?: EmployeeDocument["personal"]; bankDetails?: EmployeeDocument["bankDetails"]; previousExperience?: EmployeeDocument["previousExperience"] }) => { const employee = await Employee.findOne({ user: userId, isActive: true }); if (!employee) throw new AppError("Employee profile not found", 404); if (input.firstName !== undefined) employee.firstName = input.firstName; if (input.lastName !== undefined) employee.lastName = input.lastName; if (input.phone !== undefined) employee.phone = input.phone; if (input.professionalSummary !== undefined) employee.professionalSummary = input.professionalSummary; if (input.personal) employee.personal = { ...employee.personal, ...input.personal }; if (input.bankDetails) employee.bankDetails = { ...employee.bankDetails, ...input.bankDetails }; if (input.previousExperience !== undefined) employee.previousExperience = input.previousExperience; await employee.save(); await User.updateOne({ _id: userId }, { $set: { name: `${employee.firstName} ${employee.lastName}` } }); const refreshed = await recalculateProfileCompletion(employee.id); return withPhoto((refreshed ?? employee).toObject()); };
...
interface EmployeeUpdate { employeeId?: string; firstName?: string; lastName?: string; phone?: string; department?: string; team?: string | null; designation?: string; reportingManager?: string | null; employmentType?: EmployeeDocument["employmentType"]; officeLocation?: string; workLocation?: { geoNode: string; coordinates?: { type: "Point"; coordinates: [number, number] } } | null; role?: RoleName; status?: EmployeeDocument["status"]; personal?: EmployeeDocument["personal"]; bankDetails?: EmployeeDocument["bankDetails"] }
export const updateEmployee = async (id: string, input: EmployeeUpdate, actor: string) => {
  ...
  if (input.employeeId && input.employeeId !== employee.employeeId && await Employee.exists({ employeeId: input.employeeId, _id: { $ne: employee._id } })) throw new AppError("This employee ID is already in use", 409, "EMPLOYEE_ID_EXISTS");
  ...
  const previous = employee.toObject(); Object.assign(employee, input);
  try { await employee.save(); } catch (error: unknown) { if ((error as { code?: number }).code === 11000 && input.employeeId) throw new AppError("This employee ID is already in use", 409, "EMPLOYEE_ID_EXISTS"); throw error; }
  await User.updateOne({ _id: employee.user }, { $set: { name: `${employee.firstName} ${employee.lastName}` } });
  if (input.role) {
  ...
  const tracked = [["employeeId", "EMPLOYEE_ID_CHANGED"], ["department", "DEPARTMENT_CHANGED"], ["designation", "DESIGNATION_CHANGED"], ["reportingManager", "MANAGER_CHANGED"]] as const;
