...
const targetFilter = (scope: Awaited<ReturnType<typeof resolveSalesScope>>) => ({ employee: { $in: scope.allowedEmployeeIds }, status: { $in: ["ACTIVE", "CLOSED"] } });
export const targetPerformance = async (viewer: SessionUser, team = false) => {
  const scope = await resolveSalesScope(viewer); if (!team && scope.employeeId) scope.allowedEmployeeIds = scope.allowedEmployeeIds.filter((id) => id.toString() === scope.employeeId);
  const targets = await SalesTarget.find(targetFilter(scope)).sort({ periodStart: -1 }).populate("employee", "firstName lastName employeeId").lean();
  return Promise.all(targets.map(async (target) => {
    const employeeId = (target.employee as { _id?: unknown })._id ?? target.employee;
    const actual = (await SalesRevenueTransaction.aggregate([{ $match: { employee: employeeId, transactionDate: { $gte: target.periodStart, $lte: target.periodEnd } } }, { $group: { _id: null, total: { $sum: "$amount" } } }]))[0]?.total ?? 0;
    const commitment = await EmployeeTargetCommitment.findOne({ target: target._id, employee: employeeId }).sort({ version: -1 }).lean();
    return { target, performance: calculateTargetPerformance({ officialTarget: target.revenueTarget, actual, commitment: commitment?.committedRevenue, periodStart: target.periodStart, periodEnd: target.periodEnd }) , commitment };
  }));
};
