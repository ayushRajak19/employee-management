...
export const runResumeScreening = async (input: { jobTitle: string; jobDescription: string; files: Express.Multer.File[]; actorId: string }) => {
  ...
  for (const file of input.files) {
    ...
    const applicant = await Applicant.create({ name: normalized.candidateName, designation: input.jobTitle, originalName: file.originalname, storageProvider: stored.provider, storageKey: stored.key, format: stored.format, mimeType: file.mimetype, size: stored.size, uploadedBy: input.actorId });
