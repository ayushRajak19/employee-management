...
export const performanceAnalytics = async (input: AnalyticsInput, viewer: Viewer) => {
  ...
  const [selected, historical, kpis, goals, tasks, trackerItems] = await Promise.all([
    ...
    DailyTodo.find({ employee: { $in: ids }, date: { $gte: start.toISOString().slice(0, 10), $lt: end.toISOString().slice(0, 10) } }).select("employee status completed priority deadline durationMinutes").lean()
  ]);
  ...
  const kpiMap = valuesByEmployee(kpis); const goalMap = valuesByEmployee(goals); const trackerMap = valuesByEmployee(trackerItems); const taskMap = new Map<string, typeof tasks>();
  ...
  const rows = employees.map((employee) => {
    const key = employee._id.toString(); const snapshot = selectedMap.get(key); const previous = previousMap.get(key); const employeeKpis = kpiMap.get(key) ?? []; const employeeGoals = goalMap.get(key) ?? []; const employeeTasks = taskMap.get(key) ?? []; const employeeTracker = trackerMap.get(key) ?? []; const rated = employeeTasks.filter((item) => item.qualityRating !== undefined);
    const trackerCompleted = employeeTracker.filter((item) => item.status === "COMPLETED" || item.completed).length;
    const trackerOverdue = employeeTracker.filter((item) => item.status !== "COMPLETED" && !item.completed && item.status !== "CANCELLED" && item.deadline && item.deadline < new Date()).length;
    const department = employee.department as unknown as { _id: { toString(): string }; name: string } | null;
    return { employee: { _id: key, firstName: employee.firstName, lastName: employee.lastName, employeeId: employee.employeeId }, department: { _id: department?._id.toString() ?? "", name: department?.name ?? "Unassigned" }, score: snapshot?.totalScore ?? null, classification: snapshot?.classification ?? "Not calculated", scoreChange: snapshot && previous ? Number((snapshot.totalScore - previous.totalScore).toFixed(1)) : null, kpiAchievement: average(employeeKpis.map((item) => safeScore(item.achievement))), goalProgress: average(employeeGoals.map((item) => item.progress)), onTimeDelivery: average(employeeTasks.length ? [onTimeRate(employeeTasks)] : []), taskQuality: average(rated.map((item) => (item.qualityRating ?? 0) * 20)), learningGrowth: snapshot?.components.find((item) => item.key === "learningGrowth" && item.weight > 0)?.rawScore ?? null, completedTasks: employeeTasks.length, trackerTasks: employeeTracker.length, trackerCompleted, trackerCompletionRate: employeeTracker.length ? Number((trackerCompleted / employeeTracker.length * 100).toFixed(1)) : null, trackerOverdue, trackerUrgent: employeeTracker.filter((item) => item.priority === "URGENT").length };
  });
