...
import { AlertTriangle, Copy, Plus, Search, Trash2, UserRound, Users, X } from "lucide-react";
...
const initial = (): CreateEmployeeInput => ({ employeeId: "", firstName: "", lastName: "", officialEmail: "", phone: "", department: "", team: "", designation: "", reportingManager: "", dateOfJoining: new Date().toISOString().slice(0, 10), employmentType: "FULL_TIME", officeLocation: "", role: "EMPLOYEE", status: "ONBOARDING" });
export const EmployeesPage = () => {
    ...
    const [deleteTarget, setDeleteTarget] = useState<{ _id: string; firstName: string; lastName: string; employeeId: string } | null>(null);
...
<label className="text-sm font-medium sm:col-span-2">Employee ID<Input required maxLength={50} placeholder="e.g. MB-2026-001" className="mt-2 uppercase" {...field("employeeId")}/></label>
...
<label className="text-sm font-medium sm:col-span-2">Office location<Input className="mt-2" {...field("officeLocation")}/></label>
    ...
    {deleteTarget && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-ink/35 p-4 backdrop-blur-sm"><section role="dialog" aria-modal="true" aria-labelledby="delete-employee-title" className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start"><div className="grid size-11 shrink-0 place-items-center rounded-2xl bg-red-50 text-red-600"><AlertTriangle size={21}/></div><button type="button" aria-label="Close delete confirmation" className="ml-auto grid size-9 place-items-center rounded-xl text-slate-400 hover:bg-slate-100" onClick={() => setDeleteTarget(null)}><X size={18}/></button></div><h2 id="delete-employee-title" className="mt-5 text-xl font-semibold">Permanently delete employee?</h2><p className="mt-2 text-sm leading-6 text-slate-500"><strong className="text-ink">{deleteTarget.firstName} {deleteTarget.lastName}</strong> ({deleteTarget.employeeId}) will be removed from the application and will no longer be able to sign in.</p><div className="mt-4 rounded-xl border border-red-100 bg-red-50 p-3 text-xs leading-5 text-red-800">Their skills, assessments, attendance, tasks, reviews, goals, documents, training and other employee records will also be permanently deleted. This cannot be undone.</div>{deactivate.error && <p role="alert" className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{deactivate.error.message}</p>}<div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button type="button" variant="ghost" disabled={deactivate.isPending} onClick={() => setDeleteTarget(null)}>Cancel</Button><button type="button" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-red-600 px-4 text-sm font-semibold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60" disabled={deactivate.isPending} onClick={() => deactivate.mutate(deleteTarget._id)}><Trash2 size={15}/>{deactivate.isPending ? "Deleting…" : "Delete permanently"}</button></div></section></div>}
  </main>;
};
