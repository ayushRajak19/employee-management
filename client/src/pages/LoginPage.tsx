...
const schema = z.object({ email: z.string().email("Enter a valid official email"), password: z.string().min(8, "Password must be at least 8 characters"), tenantSlug: z.string().trim().toLowerCase().regex(/^[a-z0-9-]*$/, "Use lowercase letters, numbers and hyphens").optional() });
  ...
  return <main className="grid min-h-screen bg-white lg:grid-cols-2">
    <section className="relative hidden overflow-hidden bg-ink p-12 text-white lg:flex lg:flex-col lg:justify-between">
      ...
      <div className="relative flex items-center gap-3">
        <img src={logo} alt="MobiusBloom Endless Innovation logo" className="h-24 w-48 rounded-2xl bg-white object-contain p-2 shadow-sm" />
        <div><p className="font-semibold">MobiusBloom</p><p className="text-xs text-white/60">Employee intelligence</p></div>
      </div>
    ...
    <section className="flex items-center justify-center px-6 py-12 sm:px-12"><div className="w-full max-w-md"><div className="mb-10 flex items-center gap-3 lg:hidden">
        <img src={logo} alt="MobiusBloom Endless Innovation logo" className="h-20 w-36 rounded-2xl object-contain" />
        <span className="font-semibold">MobiusBloom Employee</span>
