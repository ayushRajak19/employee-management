...
export const AiWorkspacePage = () => {
  ...
  return <main className="flex-1 px-4 py-6 sm:px-8 sm:py-9"><div className="mx-auto w-full max-w-[1440px]">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="flex items-center gap-2 text-brand-700"><Sparkles size={17}/><p className="text-sm font-medium">MobiusEMS AI</p></div><h1 className="mt-1 text-3xl font-semibold tracking-tight sm:text-4xl">AI Workspace</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Ask permission-aware questions and turn recorded evidence into useful summaries without allowing AI to make employment decisions.</p></div><div className={`inline-flex w-fit items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold ${ready ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}>{ready ? <CheckCircle2 size={15}/> : <Settings2 size={15}/>} {configuration.isLoading ? "Checking AI setup…" : ready ? "AI Ready" : "AI setup required"}</div></div>
