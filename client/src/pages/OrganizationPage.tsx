...
import { Building2, Layers3, Pencil, Plus, Shapes } from "lucide-react";
...
export const OrganizationPage = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      ...
      if (editTarget.kind === "department") return organizationApi.updateDepartment(editTarget.item._id, body);
      ...
      if (editForm.catalogRole) body.catalogRole = editForm.catalogRole;
        ...
        {sections.map(({ key, label, icon: Icon, kind: sectionKind }) => <section key={key} className="rounded-2xl border bg-white shadow-soft">
          ...
          <div className="p-3">
            {query.isLoading ? <Skeleton className="h-32"/> : !query.data?.[key].length ? <div className="grid h-36 place-items-center text-center"><div><p className="text-sm font-medium">No {label.toLowerCase()} yet</p><p className="mt-1 text-xs text-slate-400">Create the first one to get started.</p></div></div> : <div className="space-y-1">
              {query.data[key].map((item) => <div key={item._id} className="group rounded-xl p-3 hover:bg-slate-50">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium">{item.name}</p>
                  <div className="flex items-center gap-2">
                    <button type="button" aria-label={`Edit ${item.name}`} className="grid size-7 place-items-center rounded-lg text-slate-400 opacity-0 transition hover:bg-brand-50 hover:text-brand-600 group-hover:opacity-100" onClick={() => openEdit(sectionKind, item)}><Pencil size={13}/></button>
                    <span className="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-500">{item.code}</span>
                  </div>
                </div>
    ...
    {editTarget && <div className="fixed inset-0 z-50 grid place-items-center bg-ink/35 p-4 backdrop-blur-sm">
      <form className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" onSubmit={(event) => { event.preventDefault(); updateMutation.mutate(); }}>
        ...
        <div className="mt-5 space-y-4">
          ...
          {editTarget.kind === "designation" && <label className="block text-sm font-medium">Skills for this designation<select required className="mt-2 h-11 w-full rounded-xl border bg-white px-3 text-sm" value={editForm.catalogRole} onChange={(event) => setEditForm({ ...editForm, catalogRole: event.target.value })}><option value="">Select skill catalogue</option>{query.data?.skillCatalogRoles.map((item) => <option key={item.role} value={item.role}>{item.role} ({item.skillCount} skills)</option>)}</select></label>}
