...
const solutions: Solution[] = [
  ...
  { slug:"ask-mobius", number:"05", name:"Ask Mobius AI", label:"PERMISSION-AWARE INTELLIGENCE", title:"Ask the workplace. Get the next useful move.", summary:"Mobius AI answers from current, permitted workforce data and helps organize evidence without taking employment decisions away from people.", features:["Answers permission-aware workforce questions","Works across dashboards, profiles, tasks and performance","Generates evidence-based contribution and performance reports","Identifies missing evidence without treating it as poor performance","Keeps promotion, salary, discipline and termination decisions with people"], value:"Immediate clarity from live records, with role boundaries and human judgment preserved.", accent:"#5142cb", soft:"#eceaff", Icon:Bot, visualTitle:"Ask Mobius", visualRows:["2 overdue tasks","1 review waiting","Operations workload above range","Suggested next actions"], visualMetric:"LIVE", visualMetricLabel:"GROUNDED ANSWERS" },
...
const MarketingHeader = () => {
  const { user } = useAuth();
  return <header className="sol-nav"><Link to="/welcome" className="sol-brand"><img src={logo} alt=""/><span>MobiusBloom</span><small>WORKFORCE OS</small></Link><nav><Link to="/solutions">All capabilities</Link><Link to="/solutions/employee-360">Employee intelligence</Link><Link to="/solutions/ask-mobius">AI & automation</Link></nav><div><Link to={user ? "/" : "/login"}>{user ? "Workspace" : "Sign in"}</Link><Link className="sol-nav-cta" to={user ? "/" : "/register"}>Start building <ArrowRight size={15}/></Link></div></header>;
};

const MarketingFooter = () => <footer className="sol-footer"><Link to="/welcome" className="sol-brand"><img src={logo} alt=""/><span>MobiusBloom</span></Link><p>One current source of employee information.</p><Link to="/solutions">Explore all capabilities ↗</Link></footer>;

export const SolutionsIndexPage = () => {
  useEffect(() => { window.scrollTo(0, 0); document.title = "MobiusBloom Capabilities — Complete Workforce OS"; }, []);
  return <div className="sol"><MarketingHeader/><main><section className="sol-index-hero"><p>THE COMPLETE MOBIUSBLOOM PLATFORM</p><h1>Nine capabilities.<br/><em>One workforce story.</em></h1><div>Explore every part of the employee lifecycle individually—from the employee record and daily delivery to responsible AI and enterprise control.</div></section><section className="sol-directory">{solutions.map(({slug,number,name,label,title,Icon,accent}) => <Link to={`/solutions/${slug}`} style={{"--accent":accent} as CSSProperties} key={slug}><span>{number}</span><Icon size={23}/><small>{label}</small><h2>{name}</h2><p>{title}</p><i><ArrowRight size={17}/></i></Link>)}</section><section className="sol-index-value"><p>THE BUSINESS VALUE</p><h2>One current source of employee information.</h2><div><span>Less manual administration.</span><span>Earlier workload and skill-gap visibility.</span><span>Fairer evidence-based reviews.</span><span>Management that scales across organizations.</span></div></section></main><MarketingFooter/></div>;
};
...
export const SolutionDetailPage = () => {
  ...
  const { user } = useAuth();
  useEffect(() => { window.scrollTo(0, 0); if (solution) document.title = `${solution.name} — MobiusBloom`; }, [slug, solution]);
...
const ProductVisual = ({ solution, index }: { solution: Solution; index: number }) => <div className={`sol-visual visual-${index % 3}`}>
  ...
  <div className="sol-window"><div className="sol-window-head"><i/><i/><i/><p>MobiusBloom · {solution.name}</p></div><div className="sol-window-body"><div className="sol-window-title"><span><solution.Icon size={16}/>{solution.visualTitle}</span><small>Illustrative workspace</small></div><div className="sol-metric"><small>{solution.visualMetricLabel}</small><strong>{solution.visualMetric}</strong><i/></div><div className="sol-rows">{solution.visualRows.map((row,rowIndex) => <p key={row}><span>{String(rowIndex + 1).padStart(2,"0")}</span><strong>{row}</strong><i className={rowIndex === solution.visualRows.length - 1 ? "pending" : ""}>{rowIndex === solution.visualRows.length - 1 ? "Review" : "Ready"}</i></p>)}</div></div></div>
