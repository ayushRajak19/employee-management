...
import { Check, Copy, LockKeyhole, Plus, Save, ShieldCheck, UserCog, X } from "lucide-react";
import { SECTION_ACCESS, SECTION_PERMISSIONS, type SectionPermissionName } from "@mobius-ems/shared";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { Skeleton } from "@/components/ui/Skeleton";
import { peopleOpsApi, type AdministratorCredentials, type RoleItem } from "@/features/peopleOps/peopleOpsApi";

const initialForm = { name: "", email: "", role: "HR_ADMIN" as "SUPER_ADMIN" | "HR_ADMIN" };
const roleOrder = ["SUPER_ADMIN", "HR_ADMIN", "DEPARTMENT_HEAD", "MANAGER", "EMPLOYEE"];
...
export const AdministratorsPage = () => {
  ...
  const roles = useQuery({ queryKey: ["roles"], queryFn: peopleOpsApi.roles });
  const [editingRole, setEditingRole] = useState<(RoleItem & { sections: SectionPermissionName[] }) | null>(null);
  ...
  const saveAccess = useMutation({
    mutationFn: () => peopleOpsApi.updateRoleSectionAccess(editingRole!._id, editingRole!.sections),
    onSuccess: async () => {
      setEditingRole(null);
  ...
  return <main className="flex-1 px-5 py-8 sm:px-8"><div className="mx-auto max-w-[1200px]">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-brand-700">Administration</p><h1 className="mt-1 text-3xl font-semibold">Access hierarchy</h1><p className="mt-2 text-sm text-slate-500">Decide which application sections each role can open. Super Admin access remains complete and locked.</p></div><Button onClick={() => { create.reset(); setOpen(true); }}><Plus size={16}/> Create administrator</Button></div>
    ...
    <section className="mt-5 rounded-2xl border bg-white p-5 shadow-soft"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-xl bg-violet-50 text-violet-700"><LockKeyhole size={18}/></div><div><h2 className="font-semibold">Role access levels</h2><p className="mt-1 text-xs leading-5 text-slate-500">Section access controls the sidebar and protected pages. Operational actions inside a section still follow their detailed permissions and data scope.</p></div></div>
      {roles.isLoading ? <div className="mt-5 grid gap-3 md:grid-cols-2"><Skeleton className="h-32"/><Skeleton className="h-32"/></div> : roles.error ? <p className="mt-5 rounded-xl bg-red-50 p-4 text-sm text-red-700">{roles.error.message}</p> : <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{[...(roles.data?.items ?? [])].sort((a, b) => roleOrder.indexOf(a.name) - roleOrder.indexOf(b.name)).map((role) => {
        const sections = role.permissions.filter((permission): permission is SectionPermissionName => SECTION_PERMISSIONS.includes(permission as SectionPermissionName));
        const locked = role.name === "SUPER_ADMIN";
        return <article className={`rounded-xl border p-4 ${locked ? "border-violet-200 bg-violet-50/50" : "bg-slate-50/50"}`} key={role._id}><div className="flex items-start"><div><p className="text-sm font-semibold">{role.name.replaceAll("_", " ")}</p><p className="mt-1 text-xs text-slate-500">{locked ? "All sections · locked" : `${sections.length} of ${SECTION_ACCESS.length} sections`}</p></div>{locked ? <LockKeyhole className="ml-auto text-violet-600" size={16}/> : <Button variant="secondary" className="ml-auto h-8 px-3 text-xs" onClick={() => { saveAccess.reset(); setEditingRole({ ...role, sections }); }}>Configure</Button>}</div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className={`h-full rounded-full ${locked ? "bg-violet-500" : "bg-brand-500"}`} style={{ width: `${locked ? 100 : Math.round(sections.length / SECTION_ACCESS.length * 100)}%` }}/></div></article>;
      })}</div>}
    </section>
  ...
  {editingRole && <div className="fixed inset-0 z-50 grid place-items-center bg-ink/35 p-4 backdrop-blur-sm"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start"><div><p className="text-sm font-medium text-brand-700">Access hierarchy</p><h2 className="mt-1 text-xl font-semibold">{editingRole.name.replaceAll("_", " ")}</h2><p className="mt-1 text-sm text-slate-500">Select every section this role should be able to open.</p></div><button type="button" aria-label="Close" className="ml-auto grid size-9 place-items-center rounded-lg text-slate-400 hover:bg-slate-100" onClick={() => setEditingRole(null)}><X size={17}/></button></div>
    <div className="mt-5 space-y-5">{[...new Set(SECTION_ACCESS.map((section) => section.group))].map((group) => <fieldset key={group}><legend className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">{group}</legend><div className="grid gap-2 sm:grid-cols-2">{SECTION_ACCESS.filter((section) => section.group === group).map((section) => <label className={`flex cursor-pointer items-center rounded-xl border p-3 text-sm transition ${editingRole.sections.includes(section.key) ? "border-brand-300 bg-brand-50 text-brand-800" : "hover:bg-slate-50"}`} key={section.key}><input className="size-4 accent-brand-600" type="checkbox" checked={editingRole.sections.includes(section.key)} onChange={(event) => setEditingRole({ ...editingRole, sections: event.target.checked ? [...editingRole.sections, section.key] : editingRole.sections.filter((item) => item !== section.key) })}/><span className="ml-3">{section.label}</span></label>)}</div></fieldset>)}</div>
    {saveAccess.error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{saveAccess.error.message}</p>}<div className="mt-6 flex justify-end gap-2"><Button variant="ghost" onClick={() => setEditingRole(null)}>Cancel</Button><Button disabled={saveAccess.isPending} onClick={() => saveAccess.mutate()}><Save size={15}/>{saveAccess.isPending ? "Saving..." : "Save access"}</Button></div></div></div>}
