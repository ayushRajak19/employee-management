...
export const GeographyCreateDialog = ({
  ...
  const derivedCode = level === "COUNTRY" ? code : (code || name.trim().toUpperCase().replace(/[^A-Z0-9_-]+/g, "").slice(0, 10));
  const valid = Boolean(name && derivedCode && parent);
  const submit = (event: FormEvent) => {
    event.preventDefault();
    if (!valid) return;
    const finalCode = derivedCode || "GEO";
    const lat = Number(latitude); const lng = Number(longitude);
    onSubmit({ name, code: finalCode, type: level, parent, ...(latitude && longitude && Number.isFinite(lat) && Number.isFinite(lng) ? { location: { type: "Point", coordinates: [lng, lat] as [number, number] } } : {}) });
  };
      ...
      <form role="dialog" aria-modal="true" aria-labelledby="add-geography-title" className="relative z-[2001] my-6 w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl" onSubmit={submit}>
        ...
        <div className="max-h-[calc(100vh-13rem)] overflow-y-auto p-6">
          ...
          <div className="mt-5 space-y-4">
            ...
            {name && <div className="flex items-center gap-3 rounded-xl border p-3"><span className="grid size-9 place-items-center rounded-lg bg-brand-50 text-brand-700"><MapPin size={17}/></span><div><p className="text-sm font-medium">{name}</p><p className="text-xs text-slate-400">{levelMeta.label}</p></div></div>}
          </div>
