...
export const SalesDataPage = ({ path, title }: { path: SalesDataPath; title: string }) => {
  ...
  {open && <div className="fixed inset-0 z-[2000] grid place-items-center overflow-y-auto bg-ink/35 p-4 backdrop-blur-sm"><form className="my-6 w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}><h2 className="text-xl font-semibold">Add {addLabel}</h2><p className="mt-1 text-sm text-slate-500">{sectionMeta[path].purpose}</p>
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      {path === "leads" && <><label className="text-sm font-medium">Company/account (optional)<Input className="mt-2" value={form.companyName} onChange={(event) => setForm({ ...form, companyName: event.target.value })}/></label><label className="text-sm font-medium">Country / market (optional)<Input className="mt-2" placeholder="India, UAE, Brazil..." value={form.market} onChange={(event) => setForm({ ...form, market: event.target.value })}/></label><label className="text-sm font-medium">Phone<Input required inputMode="tel" className="mt-2" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })}/></label><label className="text-sm font-medium">Email (optional)<Input type="email" className="mt-2" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })}/></label><label className="text-sm font-medium sm:col-span-2">Follow-up notes (optional)<textarea className="mt-2 min-h-20 w-full rounded-xl border bg-white px-3 py-2" maxLength={2000} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })}/></label></>}
      {path === "customers" && <><label className="text-sm font-medium">Primary contact<Input className="mt-2" value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })}/></label><label className="text-sm font-medium">Country / market (optional)<Input className="mt-2" placeholder="India, UAE, Brazil..." value={form.market} onChange={(event) => setForm({ ...form, market: event.target.value })}/></label><label className="text-sm font-medium">Phone<Input required inputMode="tel" className="mt-2" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })}/></label><label className="text-sm font-medium">Email (optional)<Input type="email" className="mt-2" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })}/></label></>}
