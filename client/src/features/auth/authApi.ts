...
export const authApi = {
  ...
  , requestPasswordReset: (body: { email: string; tenantSlug?: string }) => api.post<{ message?: string }>("/api/v1/auth/forgot-password", body), resetPassword: (body: { email: string; token: string; newPassword: string; tenantSlug?: string }) => api.post("/api/v1/auth/reset-password", body)
};
