...
export const MobiusEmsAiFloating = () => {
  const location = useLocation();
  const [open, setOpen] = useState(false);
  const [question, setQuestion] = useState("");
  const [messages, setMessages] = useState<AiChatMessage[]>([]);
  const inputRef = useRef<HTMLInputElement>(null);
  const ask = useMutation({ mutationFn: aiApi.ask, onSuccess: (data, variables) => setMessages((items) => [...items, { role: "user", content: variables.question }, { role: "assistant", content: data.answer }]) });
  useEffect(() => {
    if (!open) return;
  ...
  return <>
    {open && <button type="button" aria-label="Close MobiusEMS AI" className="fixed inset-0 z-[55] bg-ink/20 backdrop-blur-[1px] sm:hidden" onClick={() => setOpen(false)}/>}
    {open && <section role="dialog" aria-modal="true" aria-labelledby="mobius-ems-ai-title" className="fixed inset-x-3 bottom-24 z-[60] flex max-h-[min(620px,calc(100vh-7rem))] flex-col overflow-hidden rounded-3xl border bg-white shadow-2xl sm:inset-x-auto sm:bottom-24 sm:right-6 sm:w-[390px]">
      <div className="flex items-center border-b bg-gradient-to-r from-violet-50 to-white p-4"><div className="grid size-10 place-items-center rounded-xl bg-brand-600 text-white"><Bot size={19}/></div><div className="ml-3 min-w-0"><h2 id="mobius-ems-ai-title" className="font-semibold">MobiusEMS AI</h2><p className="truncate text-xs text-slate-400">Your permission-aware AI assistant</p></div><button type="button" aria-label="Close MobiusEMS AI" className="ml-auto grid size-9 place-items-center rounded-xl text-slate-500 hover:bg-white" onClick={() => setOpen(false)}><X size={18}/></button></div>
      <div className="min-h-52 flex-1 space-y-3 overflow-y-auto p-4"><div className="rounded-2xl bg-brand-50 p-3 text-sm leading-6 text-brand-900">Hi! Ask me about tasks, goals, performance evidence or how to use the portal.</div>{messages.map((message,index)=>message.role==="user"?<div key={index} className="ml-8 rounded-2xl bg-brand-600 p-3 text-sm leading-6 text-white">{message.content}</div>:<div key={index} className="rounded-2xl bg-slate-50 p-3"><AiRichResponse content={message.content} compact/></div>)}{ask.isPending&&<><div className="ml-8 rounded-2xl bg-brand-600 p-3 text-sm leading-6 text-white">{ask.variables?.question}</div><div className="flex items-center gap-2 rounded-2xl bg-slate-50 p-3 text-sm text-slate-500"><RefreshCw className="animate-spin" size={15}/> Thinking…</div></>}{ask.error&&<><div className="ml-8 rounded-2xl bg-brand-600 p-3 text-sm leading-6 text-white">{ask.variables?.question}</div><p role="alert" className="rounded-2xl bg-red-50 p-3 text-sm leading-5 text-red-700">{ask.error.message}</p></>}</div>
      <form className="flex min-w-0 gap-2 border-t bg-white p-3" onSubmit={(event) => { event.preventDefault(); const value = question.trim(); if (!value) return; ask.mutate({ question: value, employeeId: location.pathname.match(/^\/employees\/([a-f\d]{24})/i)?.[1], history: messages.slice(-8) }); setQuestion(""); }}><Input ref={inputRef} aria-label="Question for MobiusEMS AI" maxLength={500} value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Type your question…"/><Button className="size-11 shrink-0 px-0" aria-label="Send question" disabled={!question.trim() || ask.isPending}><Send size={16}/></Button></form>
    </section>}
    <button type="button" title="MobiusEMS AI" aria-label={open ? "Close MobiusEMS AI" : "Open MobiusEMS AI"} aria-expanded={open} className="fixed bottom-5 right-4 z-[65] grid size-12 place-items-center rounded-full bg-brand-600 p-0 text-white shadow-xl shadow-brand-300/50 transition hover:-translate-y-0.5 hover:bg-brand-700 focus:outline-none focus:ring-4 focus:ring-brand-200 sm:bottom-6 sm:right-6" onClick={() => setOpen((value) => !value)}>{open ? <X size={20}/> : <Bot size={20}/>}</button>
  </>;
};
...
export const DailyTodoPanel = () => {
  ...
  const create = useMutation({ mutationFn: () => todoApi.create({ date, title: title.trim(), type: "WORK", priority: "MEDIUM", durationMinutes: 30 }), onSuccess: async () => { setTitle(""); await refresh(); } });
