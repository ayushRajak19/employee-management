...
export const AskMobiusFloating = () => {
  ...
  return <>
    ...
    {open && <section role="dialog" aria-modal="true" aria-labelledby="ask-mobius-title" className="fixed inset-x-3 bottom-24 z-[60] flex max-h-[min(620px,calc(100vh-7rem))] flex-col overflow-hidden rounded-3xl border bg-white shadow-2xl sm:inset-x-auto sm:bottom-24 sm:right-6 sm:w-[390px]">
      ...
      <div className="min-h-52 flex-1 overflow-y-auto p-4"><div className="rounded-2xl bg-brand-50 p-3 text-sm leading-6 text-brand-900">Hi! Ask me about tasks, goals, performance evidence or how to use the portal.</div>{ask.isPending && <div className="mt-3 flex items-center gap-2 rounded-2xl bg-slate-50 p-3 text-sm text-slate-500"><RefreshCw className="animate-spin" size={15}/> Thinking…</div>}{ask.data && <div className="mt-3 rounded-2xl border bg-slate-50 p-3"><p className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{ask.data.answer}</p><p className="mt-3 text-[10px] text-slate-400">{ask.data.provider} · {ask.data.model}</p></div>}{ask.error && <p role="alert" className="mt-3 rounded-2xl bg-red-50 p-3 text-sm leading-5 text-red-700">{ask.error.message}</p>}</div>
    ...
    <button type="button" aria-label={open ? "Close Ask Mobius" : "Open Ask Mobius"} aria-expanded={open} className="fixed bottom-5 right-4 z-[65] inline-flex h-14 items-center gap-2 rounded-full bg-brand-600 px-4 font-semibold text-white shadow-xl shadow-brand-300/50 transition hover:-translate-y-0.5 hover:bg-brand-700 focus:outline-none focus:ring-4 focus:ring-brand-200 sm:bottom-6 sm:right-6" onClick={() => setOpen((value) => !value)}>{open ? <X size={21}/> : <Bot size={21}/>}<span className="hidden sm:inline">Ask Mobius</span></button>
  </>;
};
