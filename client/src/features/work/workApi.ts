...
export interface VoiceDraft { action: "CREATE_TASK" | "UPDATE_STATUS"; name?: string; description?: string; project?: string; assignedEmployee?: string; verbalAssigner?: string; priority?: string; complexity?: string; estimatedHours?: number; deadline?: string; task?: string; status?: string; completionNote?: string; blockerReason?: string; blockerComment?: string }
export interface VoicePreview { command: { id: string; transcript: string; language?: string; durationSeconds?: number; confidence: number; status: string }; draft: VoiceDraft; options: { projects: VoiceOption[]; employees: VoiceOption[]; tasks: VoiceOption[] } }
export interface VoiceHistory { _id: string; transcript: string; intent: string; confidence: number; status: string; actor: { name: string; email: string }; task?: { taskId: string; name: string; status: string }; createdAt: string }
export const workApi = {
  ...
  voicePreview: (audio: Blob) => { const body = new FormData(); body.append("audio", audio, "voice-task.webm"); return api.upload<VoicePreview>("/api/v1/work/voice/preview", body); },
  voicePreviewFile: (file: File) => { const body = new FormData(); body.append("audio", file); return api.upload<VoicePreview>("/api/v1/work/voice/preview", body); },
  voicePreviewText: (transcript: string) => api.post<VoicePreview>("/api/v1/work/voice/preview-text", { transcript }),
